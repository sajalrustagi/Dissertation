...
select_query="SELECT COUNT(*),YEAR(Start_Date) FROM `Disease_incidents_India` GROUP BY YEAR(Start_Date) "
select_quer_II="SELECT COUNT( * ) , Disease FROM  `Disease_incidents_India` where  Start_Date like '%2015%' GROUP BY Disease ORDER BY COUNT( * ) DESC LIMIT 0 , 30"
...
layout = Layout(
    title='Yearly Indian Outbreak Incidents',
    xaxis=XAxis(
        title='Year',
    ),
    yaxis=YAxis(
...
layout = Layout(
    title='Overall Indian Outbreak Incidents',
    xaxis=XAxis(
        title='Disease Name',
    ),
    yaxis=YAxis(
