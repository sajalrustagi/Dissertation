...
new__non_rs = dbSendQuery(mydb, "SELECT `Article`,`Stemmed_Article` FROM  `Articles_English_language_Non_corpus` GROUP BY  `Article`,`Stemmed_Article`")
...
# nondata_new <- as.data.frame(setNames(replicate(word_count+1,logical(len_non_corpus_stemmed_articles), simplify = F), seq(1:(word_count+1))))
# colnames(nondata_new)<-c(words$Word,"result")
...
#     word=words$Word[i]
#     nondata_new[[word]][j]<-grepl(word,article)
#   }
# }
# nondata_new$result<-rep(c(F),each=len_non_corpus_stemmed_articles)
# save(nondata_new, file="nondata_new.saved")
...
nondata_corpus<-docs
save(nondata_corpus, file="nondata_corpus.saved")
